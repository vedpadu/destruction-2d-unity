...
namespace DigitalRuby.AdvancedPolygonCollider
{
    ...
    public class AdvancedPolygonCollider : MonoBehaviour
    {
        ...
        public Texture2D texCreated;
        public List<Texture2D> polygonTextures = new List<Texture2D>();
        public int totalPixels;
        [HideInInspector]public List<int> polygonPixels = new List<int>();
        public List<int> doCollider = new List<int>();
        public MeshRenderer quad;
        [HideInInspector]public PolygonParameters p;
        [Serializable]
        ...
        private void Update()
        {
            ...
            {
                RecalculatePolygon();
                print(geometryDetector.tempInt + " polygonCount");
            }
        }
        ...
        public void RecalculatePolygon()
        {
            if (spriteRenderer.sprite != null)
            {
                polygonTextures.Clear();
                float startTime = Time.realtimeSinceStartup;
                p = new PolygonParameters();
                ...
                texCreated = new Texture2D(spriteRenderer.sprite.texture.width, spriteRenderer.sprite.texture.height);
                texCreated.SetPixels(geometryDetector.pixelsDetected);
                if (quad != null)
                {
                    if (Application.isPlaying)
                    {
                        quad.material.mainTexture = texCreated;
                    }
                    
                }
                
                texCreated.filterMode = FilterMode.Point;
                texCreated.Apply();
                print("Total Time: " + (Time.realtimeSinceStartup - startTime));
                print("Time Searching Hull Entrances: " + geometryDetector.totalHullEntranceSearchTime);
                print("Time Filling Polygons: " + geometryDetector.totalHoleEntranceTime);
                print("Time Filling Hull: " + geometryDetector.totalHullSearchTime);
                print("solid find time: " + geometryDetector.solidFindTime);
            }
        }
        ...
        public void PopulateCollider(PolygonCollider2D collider, ref PolygonParameters p)
        {
            try
            {
                ...
                print("time in total detectVerts/filling: " + (Time.realtimeSinceStartup - startTime));
               // print(geometryDetector.tempInt);
                polygonPixels = geometryDetector.polygonCounts;
                print(polygonPixels.Count + " poly count");
                totalPixels = geometryDetector.totalCount;
                for (var i = 0; i < geometryDetector.polygonPixels.Count; i++)
                {
                    Texture2D newTex = new Texture2D(spriteRenderer.sprite.texture.width,
                        spriteRenderer.sprite.texture.height);
                    newTex.filterMode = FilterMode.Point;
                    newTex.SetPixels(geometryDetector.polygonPixels[i]);
                    newTex.Apply();
                    polygonTextures.Add(newTex);
                }
                double startTimeColliders = Time.realtimeSinceStartup;
                int pathIndex = 0;
                doCollider = geometryDetector.doCollider;
                List<Vector2[]> list = new List<Vector2[]>();
                //print(verts.Count + " " + geometryDetector.doCollider.Count + " comprison");
                int vertInt = 0;
                for (int i = 0; i < geometryDetector.doCollider.Count; i++)
                {
                    if (geometryDetector.doCollider[i] != 1)
                    {
                        if (geometryDetector.doCollider[i] == 0)
                        {
                            ProcessVertices(collider, verts[vertInt], list, ref p, ref pathIndex);
                        }
                        vertInt += 1;
                    }
                }
                print("Time to do colliders: " + (Time.realtimeSinceStartup - startTimeColliders));
                //print(vertInt + " number verts");
        ...
        public List<Vector2[]> ProcessVertices(PolygonCollider2D collider, Vertices v, List<Vector2[]> list, ref PolygonParameters p, ref int pathIndex)
        {
