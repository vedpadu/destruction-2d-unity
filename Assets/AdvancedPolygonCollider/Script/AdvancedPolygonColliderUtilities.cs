...
namespace DigitalRuby.AdvancedPolygonCollider
{
    public class TextureConverter
    {
        ...
        public int[] indexes;
        private Color colorToShade = Color.red;
        public int tempInt = 0;
        Color blankColor = new Color(0f,0f,0f,0f);
        private bool isNewPolygonData = false;
        private Color[] origColors;
        public List<Color[]> polygonPixels = new List<Color[]>();
        public List<int> polygonCounts = new List<int>();
        public List<int> doCollider = new List<int>();
        public int totalCount;
        public double totalHoleEntranceTime = 0;
        public double totalHullSearchTime = 0;
        public double totalHullEntranceSearchTime = 0;
        public double solidFindTime = 0;
        ...
        public List<Vertices> DetectVertices(UnityEngine.Color[] colors, int width, int alphaTolerance)
        {
            ...
            pixelsDetected = new Color[colors.Length];
            origColors = colors;
            ...
            solidFindTime = Time.realtimeSinceStartup - start;
        ...
        public List<Vertices> DetectVertices()
        {
            ...
            do
            {
                ...
                if (detectedPolygons.Count == 0)
                {
                    ...
                    if (polygon.Count > 2)
                    {
                        ...
                        SearchHoleEntrance(polygon, null, true, polygonPixels.Count - 1);
                    }
                }
                ...
                if (polygon.Count > 2)
                {
                    do
                    {
                        holeEntrance = SearchHoleEntrance(polygon, holeEntrance, false, 0);
                        ...
                        if (holeEntrance.HasValue)
                        {
                            if (!blackList.Contains(holeEntrance.Value))
                            {
                                ...
                                Vertices holePolygon = CreateSimplePolygon(holeEntrance.Value, new Vector2(holeEntrance.Value.x + 1, holeEntrance.Value.y), true);
                                if (holePolygon != null && holePolygon.Count > 2)
                                {
                                    ...
                                    //SearchHoleEntrance(holePolygon, null, true, polygonPixels.Count - 1);
                    ...
                    //SearchHoleEntrance(polygon, holeEntrance, true);
                    if (isNewPolygonData)
                    { 
                        doCollider.Add(0);
            ...
            /*if (doCollider.Count == polygonPixels.Count && doCollider.Contains(2))
            {
                polygonPixels.Remove(polygonPixels[0]);
            }*/

            /*int polygonIndex = detectedPolygons.Count;
            int texIndex = polygonPixels.Count;
            for (var i = doCollider.Count - 1; i >= 0; i--)
            {
                if (doCollider[i] == 0 || doCollider[i] == 2)
                {
                    polygonIndex -= 1;
                }
                if (doCollider[i] == 1 || doCollider[i] == 0)
                {
                    texIndex -= 1;
                }
                ...
                if (doCollider[i] == 0)
                {
                    SearchHoleEntrance(detectedPolygons[polygonIndex], null, true, texIndex);
                }
            }*/
            //tempInt = detectedPolygons.Count;
        ...
        private Vector2? SearchHoleEntrance(Vertices polygon, Vector2? lastHoleEntrance, bool isForFilling, int fillingIndex)
        {
            ...
            //colorToShade = new Color(Random.Range(0f,1f), Random.Range(0f,1f), Random.Range(0f,1f));
            ...
            if (startY >= 0 && startY < height && endY > 0 && endY < height)
            {
                ...
                for (int y = startY; y <= endY; y++)
                {
                    ...
                    {
                        bool pixelFilled = false;

                        if (isForFilling)
                        {
                            for (int i = 0; i < xCoords.Count - 2; i += 2)
                            {
                                for (int x = (int) xCoords[i + 1]; x <= (int) xCoords[i + 2]; x++)
                                {
                                    if (IsSolid(x, y))
                                    {
                                        if (!pixelFilled)
                        ...
                        for (int i = 0; i < xCoords.Count; i += 2)
                        {
                            ...
                            for (int x = (int)xCoords[i]; x <= (int)xCoords[i + 1]; x++)
                            {
                                ...
                                if (IsSolid(x, y))
                                {
                                    if (isForFilling)
                                    {
                                        if (pixelsDetected[x + y * width].Equals(blankColor))
                                        {
                                            if (isNewPolygonData)
                                            {
                                                polygonPixels[fillingIndex][(int) x + (int) y * width] =
                                                    origColors[(int) x + (int) y * width];
                                                polygonCounts[fillingIndex] += 1;
                                                totalCount += 1;
                                            }
                                            pixelsDetected[(int)x + y * width] = colorToShade;
                                        }
                                        if (!pixelFilled)
                                        {
                                            if (x != (int)xCoords[i] && x != (int)xCoords[i + 1])
                                            {
                                                pixelFilled = true;
                                            }
                                        }
                                ...
                                //pixelsDetected[(int) xCoords[i] + y * width] = Color.green;
                                //pixelsDetected[(int) xCoords[i + 1] + y * width] = Color.yellow;
                            }
                        }
                        ...
                        if (isForFilling)
                        {
                            if (pixelFilled)
                            {
                                for (int i = 0; i < xCoords.Count - 1; i += 1)
                                {
                                    for (int x = (int) xCoords[i]; x <= (int) xCoords[i + 1]; x++)
                                    {
                                        if (IsSolid(x, y))
                                    ...
                                    //pixelsDetected[(int) xCoords[i] + y * width] = Color.red;
                                }
                            }
                        }
                ...
                totalHoleEntranceTime += (Time.realtimeSinceStartup - startTime);
        ...
        private Vertices CreateSimplePolygon(Vector2 entrance, Vector2 last, bool newPolygonData)
        {
            ...
            double startEntranceSearchTime = Time.realtimeSinceStartup;
            ...
            if (entrance == Vector2.zero || !InBounds(ref entrance))
            {
                ...
                if (entranceFound)
                {
                    if (newPolygonData)
                    {
                        if (pixelsDetected[((int) entrance.x) + (int) entrance.y * width].Equals(blankColor))
                        {
                            pixelsDetected[((int)entrance.x) + (int)entrance.y * width] = Color.magenta;
                            isNewPolygonData = true;
                            polygonPixels.Add(new Color[width * height]);
                            polygonCounts.Add(0);
                            polygonPixels[polygonPixels.Count - 1][((int)entrance.x) + (int)entrance.y * width] =
                                origColors[((int)entrance.x) + (int)entrance.y * width];
                            polygonCounts[polygonCounts.Count - 1] += 1;
                            totalCount += 1;
                        }
                        else
                        {
                            pixelsDetected[((int) entrance.x) + (int) entrance.y * width] = Color.green;
                        }
                    }
            ...
            else
            {
                if (IsSolid(ref entrance))
                {
                    ...
                    {
                        Vector2 v = new Vector2(entrance.x, entrance.y);
                        if (newPolygonData)
                        {
                            if (pixelsDetected[((int) entrance.x) + (int) entrance.y * width].Equals(blankColor))
                            {
                                pixelsDetected[(int)v.x + (int)v.y * width] = Color.magenta;
                                isNewPolygonData = true;
                                polygonPixels.Add(new Color[width * height]);
                                polygonCounts.Add(0);
                                polygonPixels[polygonPixels.Count - 1][(int) v.x + (int) v.y * width] =
                                    origColors[(int) v.x + (int) v.y * width];
                                polygonCounts[polygonCounts.Count - 1] += 1;
                                totalCount += 1;
                            }
                            else
                            {
                                pixelsDetected[(int)v.x + (int)v.y * width] = Color.green;
                            }
                            colorToShade = new Color(Random.Range(0f,1f), Random.Range(0f,1f), Random.Range(0f,1f));
                        }
                        current = last;
                    ...
                    else
                    {
                        ...
                        if (SearchNearPixels(false, ref entrance, out temp))
                        {
                            if (newPolygonData)
                            {
                                ...
                                colorToShade = new Color(Random.Range(0f, 1f), Random.Range(0f, 1f),
                                    Random.Range(0f, 1f));
                            }
                            current = temp;
                            entranceFound = true;
            ...
            totalHullEntranceSearchTime += (Time.realtimeSinceStartup - startEntranceSearchTime);
            #endregion
            ...
            if (entranceFound)
            {
                ...
                do
                {
                    ...
                    double startHullTime = Time.realtimeSinceStartup;
                    Vector2 outstanding;
                    ...
                    totalHullSearchTime += (Time.realtimeSinceStartup - startHullTime);
        ...
        private bool GetNextHullPoint(ref Vector2 last, ref Vector2 current, out Vector2 next)
        {
            ...
            for (int i = 0; i < closePixelsLength; i++)
            {
                ...
                if (x >= 0 && x < width && y >= 0 && y <= height)
                {
                    ...
                    {
                        Vector2 v = new Vector2(x, y);
                        if (pixelsDetected[(int) v.x + (int) v.y * width].Equals(blankColor))
                        {
                            if (isNewPolygonData)
                            {
                                polygonPixels[polygonPixels.Count - 1][(int) v.x + (int) v.y * width] =
    ...
    class PolygonPixelData
    {
        public Color[] colorData;
        public int width;
        public int height;

        public PolygonPixelData(int w, int h)
        {
            width = w;
            height = h;
